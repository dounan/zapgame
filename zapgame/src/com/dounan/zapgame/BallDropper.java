...
public class BallDropper {
  ...
  private void advanceBallDropTime() {
    nextBallDropTime += MathUtils.random(.1f);
  }
...
//      default:
        x = MathUtils.random(C.STAGE_W);
        y = C.STAGE_H + 50;
        vx = 100 + MathUtils.random(BALL_SPEED);
        if (MathUtils.randomBoolean()) vx *= -1;
        vy = -100 - MathUtils.random(BALL_SPEED);
//        break;
//    }
    int type = MathUtils.random(1) + 1;
